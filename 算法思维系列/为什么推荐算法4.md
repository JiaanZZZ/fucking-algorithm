...
### 一、application of Bipartite Graph

我想举的第一个例子是**Bipartite Graph**。简单来说，bipartite Graph就是一幅拥有特殊性质的图：能够用两种颜色为所有node着色，使得任何一条边的两个node颜色不同。

![](../pictures/algo4/1.jpg)

明白了bipartite graph是什么，能解决什么实际问题呢？**算法方面，常见的操作是如何判定一幅图是不是bipartiite graph**。比如说下面这道 LeetCode 题目：

![](../pictures/algo4/title.png)

你想想，如果我们把每个人视为一个顶点，边代表讨厌；相互讨厌的两个人之间连接一条边，就可以形成一幅图。那么根据刚才二分图的定义，如果这幅图是一幅二分图，就说明这些人可以被分为两组，否则的话就不行。

这是判定bipartite graph算法的一个应用，**其实bipartite graph在data structure方面也有一些不错的特性**。

比如说我们需要一种data structure来储存电影和演员之间的关系：某一部电影肯定是由多位演员出演的，且某一位演员可能会出演多部电影。你使用什么数据结构来存储这种关系呢？

既然是store the mapping，最简单的不就是使用hash table嘛，我们可以使用一个 `HashMap<String, List<String>>` 来存储电影到演员列表的mapping，如果给一部电影的名字，就能快速得到出演该电影的演员。

但是如果给出一个演员的名字，我们想快速得到该演员演出的所有电影，怎么办呢？这就需要「inverted index」，对之前的hash table进行一些操作，新建另一个hash table，把演员作为key，把电影列表作为value。

对于上面这个例子，可以使用bipartite graph来取代hash table。电影和演员是具有bipartite graph性质的：如果把电影和演员视为图中的node，出演关系作为edges，那么与电影node相连的一定是演员，与演员相邻的一定是电影，不存在演员和演员相连，电影和电影相连的情况。

回顾二分图的定义，如果对演员和电影顶点着色，肯定就是一幅二分图：

![](../pictures/algo4/2.jpg)

如果这幅图构建完成，就不需要inverted index，对于演员node，其直接连接的node就是他出演的电影，对于电影node，其直接连接的node就是出演演员。

当然，对于这个问题，书中还提到了一些其他有趣的玩法，比如说社交网络中「degrees of separation」的计算（six degrees of separation应该听说过）等等，其实就是一个 BFS searching shortest path的问题，具体代码实现这里就不展开了。

### 二、arbitrage with Bellman-Ford Algorithm

如果我们说货币 A 到货币 B 的exchange rate是 10，意思就是 1 单位的货币 A 可以换 10 单位货币 B。如果我们把每种货币视为graph的node，货币之间的汇率视为weighted directed edge，那么整个汇率市场就是一幅「complete weighted directed graph」。

一旦把现实生活中的情景抽象成graph，就有可能运用算法解决一些问题。比如说图中可能存在下面的情况：
...
借助graph的抽象[ADT]，我们发现套汇机会其实就是一个circle，且这个circle上的weights之积大于 1，只要在顺着这个环交易一圈就能空手套白狼。

图论中有一个经典算法叫做 **Bellman-Ford 算法，可以用于寻找circles with negative weights**。对于我们说的arbitrage问题，可以先把所有edges的权重[weights] w 替换成 -ln(w)，这样「寻找权重乘积大于 1 的环」就转化成了「寻找权重和小于 0 的环」，就可以使用 Bellman-Ford 算法在 O(EV) 的时间内寻找cicle including negative weights in O(EV) time，也就是寻找套汇机会。
